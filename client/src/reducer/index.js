...
function rootReducer(state = initialState, action) {
  switch (action.type) {
    ...
    case 'GET_BOOK_NAME_COMMENT':
      ...
      const titleBComment = bookNameComments.filter((comment) =>
        comment.books[0].title
          .toLowerCase()
          .includes(action.payload.toLowerCase())
      )
    ...
    case 'GET_WORD_COMMENT':
      ...
      const coomentWord = bookWordComments.filter((comment) =>
    ...
    case 'ADD_FAV':
      ...
      const loggedFavBooksShowed2 = loggedFavBooks2.filter(
        (book) => book.isHidden === false
      )
    ...
    case 'DELETE_BOOK_FAV':
      ...
      const loggedFavBooksShowed3 = loggedFavBooks3.filter(
        (book) => book.isHidden === false
      )
    ...
    case 'GET_STATUS_ORDERS':
      ...
      const nameStatusOrder = nameStatusOrderCopy.filter((order) =>
    ...
    case 'GET_PAYMENT_STATUS':
      ...
      const namePaymentStatus = namePaymentStatusCopy.filter((order) =>
    ...
    case 'ORDER_COMMENTS_BY_DATE':
      const commentsWithUserReducer = state.commentsAdmin.filter(
        (comment) => comment.users.length > 0
      )
    ...
    case 'ALL_HIDE':
      const booksAdminHide = state.booksAdminCopy.filter(
    ...
    case 'ALL_SHOW':
      const booksAdminShow = state.booksAdminCopy.filter(
        (book) => book.isHidden === false
      )
      return {
        ...state,
    ...
    case 'ALL_AUTHOR_HIDE':
      const authorsAdminHide = state.authorsAdminCopy.filter(
        (author) => author.isHidden === true
      )
      return {
        ...state,
    ...
    case 'ALL_AUTHOR_SHOW':
      const authorsAdminShow = state.authorsAdminCopy.filter(
        (author) => author.isHidden === false
      )
      return {
        ...state,
